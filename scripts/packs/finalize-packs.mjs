/**
 * Writes all 225-key regional pack .mjs files from locale186 modules + tail-supplement.
 * Run: node scripts/packs/finalize-packs.mjs
 */
...
for (const locale of ["hi", "kn", "bn", "gu", "mr", "ta", "te", "ml", "or", "pa", "as"]) {
  ...
  if (!tail || Object.keys(tail).length !== 39) {
    console.error(`tail-supplement ${locale}: expected 39 keys`);
    process.exit(1);
  }
  if (!tail.starOf27.includes("{n}")) {
...
for (const [locale, { data, label }] of Object.entries(LOCALES)) {
  ...
  if (missing.length) {
    console.error(`${locale}: missing ${missing.length}:`, missing.slice(0, 10).join(", "));
    process.exit(1);
  }
