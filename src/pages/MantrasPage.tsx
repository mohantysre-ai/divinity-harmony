...
const deityImages: Record<string, string> = {
  ...
  Krishna:
    "https://upload.wikimedia.org/wikipedia/commons/6/6c/Radha_Krishna.jpg",
  "Divine Mother":
    "https://upload.wikimedia.org/wikipedia/commons/5/5f/Durga_Mahishasuramardini.jpg",
  ...
  "Vedic & planetary":
    "https://upload.wikimedia.org/wikipedia/commons/9/9b/Surya_deva.jpg",
  "Vedic & universal":
    "https://upload.wikimedia.org/wikipedia/commons/1/12/Saraswati.jpg",
};
...
function deityFor(mantra: Mantra) {
  ...
  if (
    /(^| )gayatri( |$)/.test(value) &&
    !/(shiva|durga|lakshmi|saraswati|hanuman|krishna|rama|vishnu|ganesh|brahma|purusha|bhumi|hamsa)/.test(
      value,
    )
  ) {
    return "Gayatri";
  ...
  if (
    /(durga|devi|kali|tara|shakti|chandi|bhavani|annapurna|lalita|soundarya)/.test(
      value,
    )
  )
    return "Divine Mother";
  if (
    /(shiva|siva|rudra|linga|tandava|mrityunjaya|dakshinamurthy|nataraja|bhairav)/.test(
      value,
    )
  )
    return "Shiva";
  if (/(krishna|govinda|radha|madhura|bhaja govinda)/.test(value))
    return "Krishna";
  if (
    /(vishnu|narayana|narasimha|rama|gita|purusha|hayagriva|venkatesh|hari)/.test(
      value,
    )
  )
    return "Vishnu & avatars";
  if (
    /(surya|aditya|navagraha|chandra|shani|rahu|ketu|mangala|budha|shukra)/.test(
      value,
    )
  )
    return "Vedic & planetary";
...
function imageFor(mantra: Mantra) {
  ...
  return (
    mantra.imageUrl ||
    deityImages[deityFor(mantra)] ||
    labelImage(deityFor(mantra))
  );
}
function applyDeityFallback(
...
const MantrasPage = () => {
  ...
  const [script, setScript] = useState<MantraScript>(() => {
    ...
    return mantraScripts.some((item) => item.id === saved)
      ? (saved as MantraScript)
      : scriptFromBrowser();
  });
  const [scriptSource, setScriptSource] = useState(
    localStorage.getItem("preference:mantra-script")
      ? "Manual selection"
      : "Browser language",
  );
  ...
  useEffect(() => {
    void fetch("/api/favorites", { headers: deviceHeaders() })
      .then((r) => r.json())
      .then((d) =>
        setFavorites(
          new Set(
            (d.items || [])
              .filter(
                (x: { resource_type: string }) => x.resource_type === "mantra",
              )
              .map((x: { resource_id: string }) => x.resource_id),
          ),
        ),
      )
      .catch(() => undefined);
  ...
  const currentMantra =
    catalog[Math.min(currentMantraIndex, catalog.length - 1)];
  const displayedText = transliterateMantra(currentMantra.text, script);
  const deities = useMemo(
    () => ["All", ...Array.from(new Set(catalog.map(deityFor))).sort()],
    [catalog],
  );
  ...
  const explainMantra = async () => {
    ...
    try {
      const response = await fetch("/api/explain", {
        ...
        body: JSON.stringify({ text: currentMantra.text }),
      });
  ...
  const toggleFavorite = async () => {
    const id = String(currentMantra.id);
    ...
    await fetch("/api/favorites", {
      ...
      body: JSON.stringify({ resourceType: "mantra", resourceId: id, active }),
    }).catch(() => undefined);
  };
  const useRegionalScript = async () => {
    try {
      const detected = await detectRegionalScript();
      setScript(detected.script);
      ...
      const fallback = scriptFromBrowser();
      setScript(fallback);
        ...
        <main className="container mx-auto py-8">
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            <section className="lg:col-span-2">
              <article className="overflow-hidden rounded-xl bg-card shadow-lg">
                ...
                >
                  <img
                    ...
                    onError={(event) =>
                      applyDeityFallback(event, currentMantra)
                    }
                  />
                ...
                <div className="p-6">
                  ...
                  <div className="mx-auto mt-5 max-w-sm">
                    ...
                    <Select
                      value={script}
                      onValueChange={(value: MantraScript) => {
                        setScript(value);
                        setScriptSource("Manual selection");
                        localStorage.setItem("preference:mantra-script", value);
                      }}
                    >
                      <SelectTrigger>
                        <SelectValue />
              ...
              <details className="mt-4 rounded-xl border bg-card p-4">
                ...
                <AudioPlayer
                  ...
                  hasNext={currentMantraIndex < catalog.length - 1}
            ...
            <aside className="lg:col-span-1">
              ...
              <div className="max-h-[570px] space-y-3 overflow-auto pr-2">
                {visibleMantras.map(({ mantra, index }) => (
                  <Card
                    key={mantra.id}
                    role="button"
                    tabIndex={0}
                  ...
                  >
                    <CardContent className="flex gap-3 p-3">
                      <img
                        ...
                        onError={(event) => applyDeityFallback(event, mantra)}
                      />
