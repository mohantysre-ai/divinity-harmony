...
function ModuleDetailDialog({ selected, profile, module, lc, lcl, open, onOpenChange }: { selected: CulturePack; profile: StateCultureProfile; module?: { id: keyof StateDeepDive; icon: typeof Landmark; title: string; intro: string }; lc: (value: string) => string; lcl: (values: string[]) => string[]; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent style={themed(profile)} className="max-h-[88vh] max-w-3xl overflow-y-auto sm:rounded-[1.75rem]">
    <DialogHeader><div className="mb-2 flex items-center gap-3"><span className="culture-meaning-icon"><Icon className="h-6 w-6" /></span><div><p className="culture-eyebrow">{lc(selected.name)}</p><DialogTitle className="mt-1 text-left text-2xl">{lc(module.title)}</DialogTitle></div></div><DialogDescription className="text-left text-base leading-7 text-foreground/80">{lc(deepDive?.overview || module.intro)}</DialogDescription></DialogHeader>
    {deepDive ? <div className="mt-3 space-y-4">{deepDive.context && <div className="grid gap-3 sm:grid-cols-2">{deepDive.context.map((fact) => <div key={fact.label} className="rounded-2xl border bg-card p-4"><p className="culture-eyebrow">{lc(fact.label)}</p><p className="mt-2 text-sm font-semibold leading-6">{lc(fact.value)}</p></div>)}</div>}{deepDive.items.map((item) => <article key={item.name} className="rounded-2xl border bg-muted/40 p-5"><h4 className="text-lg font-bold">{lc(item.name)}</h4><p className="mt-2 text-sm leading-7 text-muted-foreground">{item.detail ? lc(item.detail) : generatedItemDetail(module.id, item.name, selected.name, lc)}</p>{item.sourceUrl && <a href={item.sourceUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-[var(--state-accent)] hover:underline">{lc("Read the source")} <ExternalLink className="h-3.5 w-3.5" /></a>}</article>)}{deepDive.sourceUrl && <a href={deepDive.sourceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--state-accent)] hover:underline">{lc(deepDive.sourceLabel || "Read the reference source")} <ExternalLink className="h-4 w-4" /></a>}</div> : <div className="mt-4 rounded-2xl border bg-muted/40 p-5"><h4 className="font-bold">{lc("Cultural highlights")}</h4><div className="mt-3 flex flex-wrap gap-2">{highlights.map((item) => <Badge key={item} variant="secondary">{item}</Badge>)}</div><p className="mt-4 text-sm leading-6 text-muted-foreground">{lc(profile.pride)}</p></div>}
  </DialogContent></Dialog>;
}

function generatedItemDetail(moduleId: keyof StateDeepDive, itemName: string, stateName: string, lc: (value: string) => string): string {
  if (moduleId === "calendar") return lc(`${itemName} is part of ${stateName}'s featured festival calendar, connecting season, devotion and community memory.`);
  if (moduleId === "rituals") return lc(`${itemName} is a living tradition in ${stateName}; practice can vary by district, community, family and lineage.`);
  if (moduleId === "journeys") return lc(`${itemName} is a starting point for understanding ${stateName}'s sacred geography, architecture and pilgrimage traditions.`);
  if (moduleId === "language") return lc(`${itemName} carries part of ${stateName}'s language, calendar, script or oral memory.`);
  if (moduleId === "arts") return lc(`${itemName} expresses ${stateName}'s cultural knowledge through material, movement, colour, sound or skilled making.`);
  return lc(`${itemName} offers a doorway into ${stateName}'s geography, seasons, hospitality and everyday food memory.`);
}
