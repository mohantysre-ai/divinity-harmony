...
const FeaturedCarousel = () => (
  <section className="relative overflow-hidden bg-stone-950 py-20 text-amber-50 sm:py-24">
    <div className="home-stars absolute inset-0 opacity-50" aria-hidden="true" />
    <div className="container relative mx-auto px-5 lg:px-8">
      <div className="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-amber-400">A sacred rhythm for every day</p><h2 className="mt-3 max-w-xl font-sanskrit text-3xl font-bold tracking-tight sm:text-4xl">Choose the moment you need.</h2></div>
        <p className="max-w-md text-sm leading-6 text-stone-400">No noise, no complicated menus—just a direct path to listening, darshan, and timeless knowledge.</p>
      </div>
      <div className="grid gap-5 lg:grid-cols-3">
        {moments.map((moment, index) => (
          <Link key={moment.title} to={moment.href} className={`home-moment-card group ${moment.tone}`} style={{ animationDelay: `${index * 120}ms` }}>
            <div className="flex items-start justify-between"><div className="rounded-2xl border border-white/10 bg-white/10 p-3"><moment.icon className="h-6 w-6" /></div><ArrowUpRight className="h-5 w-5 transition-transform duration-300 group-hover:-translate-y-1 group-hover:translate-x-1" /></div>
